...
- **GitHub Pages 分析・予測ダッシュボード**: 監視履歴データを元に、**在庫増の遷移（曜日・時間帯トレンドや次回入荷予測）** および **在庫減の遷移（購入/処分タイミング、最短時間・中央値滞留時間）** を美しいダッシュボードとして自動公開・更新します。
- **Fork-Safe なデータ分離設計**: 監視生データ（`history` ブランチ）およびレポート成果物（`gh-pages` ブランチ）は `main` ブランチから完全に隔離されているため、リポジトリを Fork した場合でも本家（upstream）との同期時に一切コンフリクトが発生しません。
...
- **データ分離コミット**: 結果を独立したデータ専用ブランチ（`history` ブランチ）に自動保存します。
- **GitHub Pages 自動更新**: 新しいデータの検出時（または手動実行時）に自動的に分析エンジンが起動し、入荷トレンド・完売滞留時間を集計して独立した `gh-pages` ブランチ（および GitHub Pages）にダッシュボードを自動公開・更新します。`main` ブランチにはコードのみが保持されるため、リポジトリを Fork（フォーク）した場合でも本家からの更新同期（Sync Fork）がコンフリクトなく行えます。
