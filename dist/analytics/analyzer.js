 ...
 */
export function getJstInfo(date) {
...
export function parseMetaContent(content) {
    ...
    for (const line of lines) {
        ...
        if (key === "Detected at")
            meta.detectedAt = val;
        else if (key === "Detected at (JST)")
            meta.detectedAtJst = val;
        else if (key === "URL")
            meta.url = val;
 ...
 */
export function createProductSignature(p) {
    const parts = [
        (p.price || "").trim(),
 ...
 */
export function loadTargetSnapshots(targetDir, targetName) {
    ...
    function walkDir(dir) {
        ...
        if (hasData) {
            ...
            let dt = null;
            if (meta?.detectedAt)
                dt = parseTimestamp(meta.detectedAt);
            if (!dt && meta?.detectedAtJst)
                dt = parseTimestamp(meta.detectedAtJst);
            if (!dt)
                dt = parseTimestamp(dirName);
            if (dt) {
                let products = [];
                try {
 ...
 */
export function computeStockEvents(snapshots) {
    const events = [];
 ...
 */
export function computeStockInTrend(events) {
 ...
 */
export function computeStockOutStats(events) {
 ...
 */
export function generatePrediction(stockIn, stockOut) {
    // 曜日トップ2
    ...
    let confidence = "Low";
    if (stockIn.totalAdded >= 20)
        confidence = "High";
    else if (stockIn.totalAdded >= 5)
        confidence = "Medium";
...
export function analyzeHistory(historyDir, targets, options) {
    ...
    for (const target of targets) {
        ...
        if (snapshots.length > 0) {
            ...
            if (!allMinDate || firstSnap.timestamp < allMinDate)
                allMinDate = firstSnap.timestamp;
            if (!allMaxDate || lastSnap.timestamp > allMaxDate)
                allMaxDate = lastSnap.timestamp;
        }
