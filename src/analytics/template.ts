...
import type { FullAnalysisReport, TargetAnalysis, StockEvent } from "./types.js";
...
function renderBarChart(
  items: { label: string; value: number; percentage: number; highlight?: boolean }[],
  unit = "件"
): string {
...
function renderEventTable(events: StockEvent[], type: "in" | "out"): string {
  if (events.length === 0) {
...
export function renderReportHtml(report: FullAnalysisReport): string {
  ...
  <main class="container">
    ...
    <div class="target-tabs" id="target-tabs">
      ...
      ${report.targets
        .map(
          (t) =>
            `<button class="tab-btn" data-target="${escapeHtml(t.targetName)}">${escapeHtml(t.targetName)}</button>`
        )
        .join("")}
    </div>
      ...
      <div class="grid-2col">
        <div>
          ...
          <div id="chart-in-days">
            ${renderBarChart(
              overall.stockIn.byDayOfWeek.map((d) => ({
      ...
      <div class="grid-2col">
        <div>
          ...
          <div id="chart-duration-buckets">
            ${renderBarChart(
              overall.stockOut.durationBuckets.map((b) => ({
                label: b.label,
                value: b.count,
